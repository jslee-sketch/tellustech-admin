...
- **v2.9.1 · 2026-05-04**: 100-scenario E2E seed + 4 financial-statement enrichment + 5 client bugfixes.
  ...
  - **Cash-flow classification**: `src/lib/financial-statements.ts` `computeCashFlow` — uses each entry's counter-account to classify INVESTING (211/212/213/214/228) vs FINANCING (311/338/411/412/421/414) vs OPERATING. Client (`cash-flow-client.tsx`) renders all three sections with subtotals.
  - **5 client bugfixes**: ① `j.data?.rows` double-wrapper assumption → `j.rows ?? j.data?.rows` fallback (TB/IS/BS/CF/Profitability/Closings, 7 screens). ② URL `?period=`/`?asOf=` query ignored → `useEffect` mount-time URL→state init. ③ `cache: "no-store"` explicit (defeats stale browser cache). ④ `useEffect` deps `[period]`/`[asOf]` (auto reload on input change). ⑤ Print rendered numbers as 0 → `@media print` `* { color: #000 !important; -webkit-print-color-adjust: exact; }` + force-black Tailwind colour classes + monospace font.
  - **Seed idempotency**: `entryDate` normalisation to 2026-04-15 via updateMany; bank balance recomputed from confirmed cash txns; EXP-068 reimburse re-entry guard; SLS-006 leftover handling.
  - **Auto-verification**: `scripts/dev-only/verify-100.ts` (11/11 PASS) + `http-check.sh` (65/65) + `api-data-check.sh` (18/18). Chrome live 7-screen PASS (dashboard/sales/TB/IS/BS/CF/cash-dashboard).
  - i18n vi/en/ko +2 keys (fs.subtotal, fs.amount).
- **v2.9.0 · 2026-05-03**: PWA offline + points expiry policy.
  - **PWA**: Root-scope Service Worker `/sw-erp.js` + manifest + icons, offline banner + install prompt, page-cache whitelist (dashboard/yield/stock/cash/profitability), auto-reload on reconnect.
  - **Points expiry**: `PortalPoint` gains `expiresAt`/`expiredAt`/`consumedFromId`. `PointConfig.expiryMonths` (default 24). Auto-set on grant. FIFO consumption by oldest grant first. Cron `/api/jobs/points-expiry` (daily 07:00 KST) expires past-due rows + emits `PORTAL_POINTS_EXPIRY_SOON` 30 days ahead (28 default rules × 2 companies). `/api/portal/points` returns `expiring30d` + `nextExpireAt`.
