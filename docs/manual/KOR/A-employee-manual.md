...
- **v2.9.5 · 2026-05-04**: 재고 3화면 풍부화 — 옵션·평균매입단가·가용수량·현황 컬럼 추가.
  - **DB 마이그레이션**: `PurchaseItem.options String?` + `InventoryItem.options String?` 추가 (`prisma db push --accept-data-loss`). 옵션은 매입 등록 라인에서 입력 → 입출고/재고 화면 어디서나 노출.
  - **실시간 재고현황** (`/inventory/stock` 실시간 탭): 컬럼 +3종 — **가용수량** (자사 + Internal 창고 + currentLocation NULL + status NORMAL) / **평균 매입단가** (PurchaseItem 가중평균: Σ amount / Σ quantity) / **재고 금액** (평균단가 × 현재재고).
  - **S/N별 재고현황**: detail row +2 컬럼 — **옵션** (InventoryItem.options) / **현황** (재고/렌탈/수리/교정/데모) — `currentLocationClientId` NULL 이면 "재고", 있으면 마지막 OUT 트랜잭션의 `referenceModule` 매핑 (RENTAL→렌탈/REPAIR→수리/CALIB→교정/DEMO→데모). currentLocationLabel 동시 표시.
  - **입출고 현황**: S/N 컬럼 옆에 **옵션 배지** (InventoryItem.options 자동 lookup). 검색이 단일 S/N 정확 매칭 시 상단에 **단일 S/N 이력 detail card** (S/N + 옵션 + 품목 + IN N회 / OUT N회).
  - **API 보강**: `/api/purchases` 가 라인의 `options` 받아 PurchaseItem + InventoryItem 마스터 양쪽에 저장. `ensureInventoryItemOnReceipt` 가 options 받아 마스터 생성/resurrect 시 채움.
- **v2.9.4 · 2026-05-04**: 입출고 등록 로직 재구성 — S/N 우선 + 마스터 상태 검증 + 정책 4종 적용.
  - **폼 순서 재배치**: 유형/시나리오 → 라인 → 헤더 (역순) 에서 라인 우선으로 재구성. 라인 default 빈 배열 (이전 1행 자동) — 사용자가 명시적 [+ 라인 추가] 해야 함.
  - **S/N → 품목 자동 매핑** (`SerialCombobox.onPick` 활용): S/N 픽 시 마스터(`/api/inventory/sn/[sn]/state`) 조회 → itemId/itemCode/itemName 자동 셋팅 + readonly lock. 직접 S/N 타이핑 후 blur 시도 동일 흐름. 이전: 품목 → S/N 검색 (반대 방향) 단방향 흐름이라 마스터 상태 검증 무력화.
  - **마스터 상태 배지**: 라인의 S/N 옆에 `OWN_IN_STOCK / OWN_AT_EXTERNAL / EXTERNAL_IN_STOCK / EXTERNAL_AT_VENDOR / ARCHIVED / NEW` 색상 배지 표시 — 사용자가 시나리오 선택 전 자산 상태 파악.
  - **라인 검증** (정책 G): 모든 라인 itemId + S/N + 수량 ≥ 1 필수. 위반 행 번호 명시 alert.
  - **정책 D — archived 후 재입고 (잔량 0)**: `ensureInventoryItemOnReceipt` 가 exists.archivedAt !== null 인 경우 archivedAt:null 로 resurrect + 새 ownerType/inboundReason 갱신. itemId 변경 차단 (정책 B).
  - **정책 E — 외부 자산 archived 후 재입고/이동 OK, 출고 NG**: `/api/inventory/sn/[sn]/state` ARCHIVED + EXTERNAL_CLIENT 인 경우 IN/RENTAL/BORROW + IN/REPAIR/REQUEST + IN/CALIB/REQUEST + IN/DEMO/BORROW + TRANSFER/RENTAL/OTHER 추천. OUT 류는 Bulk API 에서 차단 (`external_archived_no_out`).
  - **정책 H — 입출고 현황 입고창고 폴백**: 컬럼이 toWarehouseCode → toClientCode → clientCode 우선순위로 표시. NULL 인 옛 OUT 47건 (이전 toWarehouse 미입력) 도 거래처 정보로 "입고창고" 자동 채움.
  - **서버 측 S/N-품목 일치 검증** (정책 B): `/api/inventory/transactions/bulk` 가 입력 itemId vs 마스터 itemId 비교 후 불일치 시 400 + `sn_item_mismatch`. 정책 F (TRANSFER 미등록 S/N 허용) 보존.
- **v2.9.3 · 2026-05-04**: 입출고 OUT 도착창고 필드 + QR 라벨 마스터 검증 + S/N 정합성 backfill.
  - **입출고 OUT 도착창고**: `/inventory/transactions/new` 의 OUT 모드에 **`도착창고`** (`field.toWh`) 필드 추가. 자사 Internal 창고 + EXTERNAL 외부 거래처 창고 모두 옵션에 노출. 라벨 "(외부)" 접미사로 구분. 렌탈 출고 등 자사→외부 흐름에서 입고지(=도착창고)를 명시적으로 추적 가능.
  - **입출고 IN 도착창고**: 외부 창고 옵션 노출 추가 (외부 위탁 자산이 외주처에서 외부 창고로 이동하는 케이스 지원).
  - **QR 라벨 마스터 검증**: `/inventory/labels` 의 [+ 추가] 버튼이 InventoryItem 마스터를 조회 (`/api/inventory/sn/[sn]/state`) → 등록되지 않은 S/N 차단 + 선택한 품목과 마스터 품목 불일치 시 경고. `useEffect` QR 생성도 S/N 비어 있으면 스킵. 즉, 매입/입고 거치지 않은 임의 라벨 생성 불가.
  - **S/N 정합성 backfill** (`scripts/dev-only/backfill-orphan-sn.ts`): 옛 E2E/mass-import 시드의 orphan S/N 123건을 자동으로 InventoryItem 마스터에 등록. WH-BACKFILL holding warehouse + ITM-BACKFILL placeholder 품목 자동 생성. ItContractEquipment / PurchaseItem / SalesItem / InventoryTransaction / AsTicket / AsDispatch / TmRentalItem 모두 추적 가능.
  - **production 시드 + 검증**: 100 시나리오 시드를 운영 DB에 적용 (created=308). S/N 정합성 검증 19/19 PASS (이전 4 fail → 0 fail). Chrome 라이브 7화면 (대시보드/시산표/IS/BS/CF/자금현황판) 모두 PASS.
  - i18n vi/en/ko +3 키 (`label.snRequired`, `label.snNotInMaster`, `label.snItemMismatch`).
- **v2.9.2 · 2026-05-04**: 재무제표 누적(YTD) + 자금관리 UX 일괄 보강.
  - **재무제표 4종 누적 모드**: 시산표/손익/현금흐름이 단월 → **회계연도 1월 1일~기준월말까지 누적** 변경 (`computeTrialBalance`/`computeCashFlow` from = `Date.UTC(year, 0, 1)`). 거래처 수익성도 동일하게 누적. 라벨 "기간" → "누적 기준월 (YTD)".
  - **매출 화면**: 컬럼별 정렬(↑↓) 토글 + 시작/종료일 검색 + 기존 단계/상태 dropdown 유지.
  - **계좌 관리**: 입금/출금/이체 등록 시 확인 모달 (계좌·금액·설명 노출). 자금흐름 직접 영향 거래라 사용자 확인 필수.
  - **입출금내역(`/finance/cash-transactions`)**: 별도 클라이언트 컴포넌트로 분리. 검색(코드/계좌/설명/카테고리) + 거래유형/상태 dropdown + 시작/종료일 + 컬럼 정렬 + 페이지 크기(10/30/50/100) + 이전/다음 페이징.
  - **자금현황판**: 월별추이 0 표시 버그 수정 — `BankAccountMonthlySnapshot` 빈 경우 CashTransaction 직접 집계 폴백. `?monthsBack=N` 쿼리(1~24개월) 지원.
  - **비용**: 검색(코드/메모/타입) + 타입 dropdown + 시작/종료일 + 페이지 크기·이전/다음 페이징 (기존 상태 필터 유지).
  - **거래처 수익성**: 누적 모드 + 정렬(거래처/매출/직접비/공헌이익/순이익/이익률) + 검색 + 페이징.
  - **대시보드**: 100 시드 반영 KPI 3종 추가 — 자금 잔고 합계, 활성 IT 계약 수, 활성 TM 렌탈 수.
  - **layout.tsx 버그 수정**: `<script>` 태그 → `next/script` (Next.js 16 hydration 에러 해소).
  - i18n vi/en/ko 키 추가: `fs.cumulative`, `fs.cumulativeHint`, `col.dateFrom`, `col.dateTo`, `page.size`, `page.prev`, `page.next`, `confirm.cashTxn`, `dash.kpi.bankBalance`, `dash.kpi.itContractsActive`, `dash.kpi.tmRentalsActive` 등 11개.
- **v2.9.1 · 2026-05-04**: 100 시나리오 E2E 시드 + 재무제표 4종 보강 + 클라이언트 5건 버그 수정.
  - **100 시나리오 시드 (`scripts/dev-only/seed-full-100.ts`)**: 거래처 10 + 직원 10 + 창고 5 + 품목 15(BOM/호환) + 매입 10(1.7B) + 매출 10(172.4M) + IT계약 5+장비9 + TM 3 + AS 12 + 비용 8 + 은행 4 + Cash 30 + 분개 60+ + 급여 10 + UC 15 + SNMP 27 + Yield 9 + 포인트 3 + 알림 16 + PR 20. 모두 prefix `-100-` 으로 기존 시드 보존하며 추가만.
  - **재무제표 4종 콘텐츠 풍부화**: 시산표 활성 계정 9개 → **30개**. 매출 511 → 5111(상품매출)/5113(서비스매출)/5117(임대수익) 사업유형별 분기. 비용 642 → 6423(사무용품)/6424(여비교통)/6425(접대)/6426(임차)/6428(기타현금)/6429(공과·통신) 세분화. VAT 분개 (3331 부가세예수금 / 133 매입세액 10%). 211 유형자산 + 152 원재료 + 153 공구비품 분리. 411 자본 200M + 421 이익잉여금 30M + 338 차입 100M (FINANCING). 711 기타수익 + 515 금융수익 + 811 기타비용 영업외 활성. 매출원가(632) 라인별 인식.
  - **현금흐름표 분류 로직**: `src/lib/financial-statements.ts` `computeCashFlow` — 동일 분개의 상대 계정으로 `INVESTING_ACCS(211/212/213/214/228)` / `FINANCING_ACCS(311/338/411/412/421/414)` / `OPERATING` 자동 분류. 클라이언트(`cash-flow-client.tsx`)에 투자/재무 섹션 + 소계 행 렌더링 추가.
  - **클라이언트 버그 5건 수정**: ① `j.data?.rows` 이중래퍼 가정 → `j.rows ?? j.data?.rows` (TB/IS/BS/CF/Profitability/Closings 7개 화면). ② URL `?period=`/`?asOf=` 쿼리 무시 → `useEffect` mount 시 URL→state 초기화. ③ `cache: "no-store"` 명시 (옛 응답 캐시 차단). ④ `useEffect` deps에 `[period]`/`[asOf]` 추가 (입력 변경 시 자동 reload). ⑤ 손익/시산/BS/CF 프린트 시 숫자 0 표시 → `@media print` `* { color: #000 !important; -webkit-print-color-adjust: exact; }` + Tailwind 색상 클래스 강제 검정 + 폰트 mono 강제.
  - **시드 멱등성 강화**: JE-100-* `entryDate` 일괄 04-15 보정 (`updateMany`), BankAccount.currentBalance 트랜잭션 합계 재계산, EXP-068 환급 재진입 가드, SLS-006 잔재 처리.
  - **검증 자동화**: `scripts/dev-only/verify-100.ts` (정합성 11/11) + `http-check.sh` (HTTP 65/65) + `api-data-check.sh` (데이터 18/18). Chrome 라이브 7화면 PASS — 대시보드/매출/시산표/손익/재무상태표/현금흐름표/자금현황판.
  - i18n vi/en/ko 2 키 추가 (fs.subtotal, fs.amount).
- **v2.9.0 · 2026-05-03**: PWA 오프라인 + 포인트 만료 정책 신설.
  - **PWA**: `/sw-erp.js` (root scope) — 정적 자산 cache-first, HTML stale-while-revalidate, API/_next/data network-only. `manifest-erp.webmanifest` (앱 이름 Tellustech ERP) + 192/512 SVG 아이콘. `OfflineBanner` 컴포넌트 — `online`/`offline` 이벤트 감지 + 마지막 동기화 시각 + `beforeinstallprompt` 캡처 + 설치 프롬프트. 오프라인 시 캐시 hit 허용 화면 화이트리스트 (대시보드/적정율/재고현황/자금현황판/수익성). 온라인 복귀 시 자동 reload.
  - **포인트 만료**: `PortalPoint.expiresAt`/`expiredAt`/`consumedFromId` 추가. `PointConfig.expiryMonths` (기본 24). `grantPoints()` 가 적립 시 `expiresAt = today + N months` 자동 설정. `fifoConsumePoints()` — FIFO 차감(가장 오래된 적립행부터 소진), 다행 분할 + `consumedFromId` 트레이스. `deductPoints()` 도 FIFO 사용.
  - **만료 cron**: `/api/jobs/points-expiry` (매일 07:00 KST) — `expiresAt < today` 행을 `POINT_EXPIRED` reason 으로 음수 차감 + 원본 `expiredAt` 셋팅. 30일 후 만료 예정 → `PORTAL_POINTS_EXPIRY_SOON` 이벤트 (영업 담당). 새 NotificationEventType + 기본 규칙 1건 추가 → 28 × 2 회사.
  - **포탈 포인트 화면**: `/api/portal/points` 응답에 `expiring30d` + `nextExpireAt` 추가 — 사용자 잔액 옆에 "이 중 X,XXXd 는 30일 내 만료" 표시 가능.
  - i18n vi/en/ko 7 키 추가.
- **v2.8.1 · 2026-05-03**: 잔여 4건 일괄 처리.
  - **재고 Phase 2 E2E 31/31 PASS** — 알림 시스템 도입 후에도 회귀 0건.
  - **간접비 자동 배분** (`/api/finance/profitability`): `targetClientId` 가 없는 공통 비용 합계를 매출 비율(REVENUE_RATIO)로 거래처별 자동 안분. 거래처별 수익성 화면에 indirectCostAlloc 실제값 노출 (이전 0 고정 → REVENUE_RATIO 분배).
  - **알림 hook 4종 보강**: SALES_FINANCE_CFM_REQUEST (영업 → 재경 매출 발행 시), SALES_FINANCE_CFM_DONE (재경 → 영업 CFM 완료 시), EXPENSE_REIMBURSE_APPROVED (환급 승인 시 — Expense.createdById 부재로 ROLE/ADMIN 한정), AS_TICKET_ASSIGNED (담당 기사 변경 시).
  - **AI 뉴스 월간 캡**: `PORTAL_NEWS_MONTHLY_CAP` 환경변수 (기본 8) — 같은 월 PortalPost 카운트 ≥ cap 이면 cron 호출 시 429 반환. `?force=1` 로 ADMIN 일회성 우회.
- **v2.8.0 · 2026-05-03**: **통합 알림 시스템** 신설 — Email + Zalo + ERP Chat 3채널.
  - 신규 모델: `NotificationRule` (이벤트 → 대상 → 채널 매핑), `NotificationDelivery` (채널별 발송 이력 + 재시도 카운터). `Notification.eventType`/`linkedModel`/`linkedId` 추가. `Employee.personalEmail`/`zaloId`/`notifyEmail`/`notifyZalo`/`notifyChat` 5필드.
  - 30개 `NotificationEventType` 정의 — AS/매출CFM/비용환급/일정/미수금 D-7·D-3·연체/미지급금 D-3/계약 만료 D-30·D-7/라이선스/회계마감 리마인더/포탈 AS·소모품·견적·피드백·추천/사용량 CFM 완료·미응답 D+3/SNMP agent 끊김/토큰 만료/적정율 의심/현금 부족 등.
  - 기본 규칙 27개 자동 시드 (TV/VR 양쪽). `targetType`: INDIVIDUAL/TEAM/ROLE/ALL/ASSIGNEE/ASSIGNEE_MANAGER/CLIENT_PIC. 채널은 ADMIN 화면에서 on/off + 직원 개인 설정에서 on/off (논리곱).
  - **이메일** (`src/lib/notify/email-sender.ts`): Gmail SMTP nodemailer. `GMAIL_USER` + `GMAIL_APP_PASSWORD` 환경변수 기반. 브랜드 HTML 템플릿(`wrapBrandHtml`) — TTS 로고 + 본문 + CTA 버튼 + 푸터.
  - **Zalo** (`src/lib/notify/zalo-sender.ts`): OA API v3 — `ZALO_OA_ACCESS_TOKEN` 미설정 시 자동 스킵 (silent skipped:no_zalo_oa_token 로그).
  - **ERP Chat** (`src/lib/notify/chat-sender.ts`): 시스템 SYSTEM 사용자 자동 생성 + 직원 1:1 DIRECT 방 자동 확보 + ChatMessage 발송. `originalLang: "KO"`.
  - **통합 디스패처** (`src/lib/notify/dispatcher.ts`): 이벤트 → Rule lookup → `resolveRecipients()` (7 모드) → 템플릿 변수 치환 (`{clientName}`, `{amount}` 등) → Notification 생성 + 채널별 발송 + Delivery 이력 저장. 실패해도 호출 측 트랜잭션 무영향.
  - 기존 모듈 hook 5곳: 포탈 AS/소모품/견적, AS dispatch 완료. cron 트리거 2개: `/api/jobs/notification-scheduler` (매일 06:00) + `/api/jobs/notification-retry` (매시간, FAILED + retryCount<3).
  - 신규 화면 3종: `/admin/notification-rules` (ADMIN — 27 규칙 채널 토글 + 템플릿 편집), `/admin/notification-history` (ADMIN — 발송 이력 + 채널/상태 필터), `/notifications` (전 직원 — 본인 알림 목록, 안 읽음 필터 + 읽음 표시 + 모두 읽음). `/settings/notifications` (개인 설정 — personalEmail/zaloId + 3 채널 토글).
  - 사이드바 헤더에 🔔 벨 + 안 읽음 갯수 배지 (60초 폴링). 사이드바 시스템 그룹에 알림 규칙·발송 이력 +2 메뉴.
  - 직원 등록·수정 폼에 `personalEmail` + `zaloId` 필드. 직원 API GET/PATCH 모두 신규 필드 처리.
  - i18n 30+ 키 vi/en/ko.
- **v2.7.1 · 2026-05-03**: UX 빈틈 보강.
  - `/finance/cash-transactions` 페이지 헤더에 [+ 신규 입출금] 버튼 — `/finance/accounts` 행 액션으로 안내.
  - Expense 환급 승인 — `window.prompt(계좌 ID)` → 모달 + 계좌 select dropdown 으로 개선. 계좌 미등록 시 안내 메시지.
  - i18n 6 키 추가 (vi/en/ko).
- **v2.7.0 · 2026-05-03**: 8개 후속 작업 일괄 — RBAC + 매뉴얼 통합 + 기술부채 + 기사별 통계 + 즐겨찾기 서버동기 + Sales FK 백필.
  - **RBAC 가드** (`src/lib/rbac.ts` 신규): 재경 19 페이지 페이지단 `checkFinanceAccess()` — EMPLOYEE+ 는 비용/PR 접근, MANAGER+ 는 자금/원장/재무제표, ADMIN-only API 액션 (period-close `close`/`reopen`, account-mappings PUT). CLIENT 는 모두 /portal redirect.
  - **매뉴얼 본문 통합**: A 매뉴얼 8.3 자금관리/8.3.4 비용 강화/8.4 PR 모달 계좌/8.5 회계원장. B 매뉴얼 3-A 회계 설정/3-B 매핑 관리/3-C 재무제표/3-D 마감/3-E 비용센터·수익성. 3 langs 모두.
  - **즐겨찾기 서버 동기**: `User.sidebarFavorites` 컬럼 + `/api/user/favorites` GET/PUT. localStorage 1회 자동 마이그레이션 후 캐시 제거. 브라우저/PC 바뀌어도 유지.
  - **기사별 통계 정확화**: `/api/admin/yield-analysis/tech-stats` 신규 — `AsDispatchPart.asDispatch.dispatchEmployee` + `targetContractId` 기반 진짜 기사별 집계. 거래처 fallback 폐기. 컬럼: 기사 / 담당 계약 / 사용 부품 / 평균 적정율 / 의심 계약 수.
  - **Sales→계약 FK 백필**: `scripts/backfill-sales-contract.ts` — itContractId/tmRentalId NULL 인 RENTAL 매출을 거래처+기간으로 자동 매칭. 매칭 불가 목록 출력 (수동 매칭 안내).
  - **기술 부채 정리**: portal-news-generate auth 진단 console.log 제거. E2E 14 스크립트 → `scripts/dev-only/`. i18n 중복 0건. NIIMBOT B21 라벨 (50×70mm 단일 규격, page-break, 다크모드 무력화) 이미 완전 구현 확인.
  - **재고 Phase 2 E2E**: 31/31 PASS (이미 v2.6.1).
- **v2.6.1 · 2026-05-03**: 재고 리팩토링 **Phase 2 E2E 검증** + 시드 분리.
  - `scripts/seed-inventory-e2e.ts` 분리 — 거래처 6 + 창고 2 + 품목 5 + 사전 자사 자산 마스터 4 (R001/RP02/CL02/DM02). 멱등 cleanup + 단독 실행 가능.
  - 진리표 20행 시나리오 + 3 추가 + 4 cross-verify = **31/31 PASS** (`npx tsx scripts/test-inv-e2e.ts`). InvTxn 행, InventoryItem 마스터 동작(NEW/MOVE/ARCHIVE/TRANSFER_LOC), ownerType + currentLocationClientId + archivedAt, 자사 onHand 합계, BASE_RULES 룩업, 자동 매입/매출 PR DRAFT 모두 검증.
  - Chrome 검증: `/inventory/transactions/new` cascading select — IN(11 콤보) / OUT(14 콤보) / TRANSFER(5 콤보) 정상 분기. TRANSFER 첫줄=내부재고이동(창고↔창고), 2~5줄=외부 패스스루(거래처↔거래처) 모드 자동 전환.
  - i18n 누락 보강 — `field.fromWarehouse` / `field.toWarehouse` 키 vi/en/ko 추가 (TRANSFER 내부이동 모드에서 노출되던 i18n key string 해소).
- **v2.6.0 · 2026-05-03**: 재경 **Layer 5 — AccountingConfig + 사이드바 정렬 + E2E 21단계** 완성.
  - **AccountingConfig** (회사별 1행) — 회계 표준 프리셋 3종(VAS/K_IFRS/IFRS) + 회계연도 시작월(JAN/APR/JUL/OCT) + 보고통화(VND/KRW/USD) + 기본 VAT률 + 보고서 기본 언어 + 발생주의 toggle + 자동분개 toggle + 마감 가드 toggle. 신규 화면 `/finance/accounting-config` (ADMIN/MANAGER 만) — preset 3카드 + 세부 설정 폼 + [저장].
  - **사이드바 재경 그룹 분할** — 단일 16-메뉴 그룹을 3개 sub-group 으로 정리: `재경 · 자금관리`(8 메뉴) / `재경 · 회계원장`(4 메뉴, AccountingConfig 포함) / `재경 · 재무제표`(5 메뉴 — 시산표·PL·BS·CF·회계마감). 시각적 그룹 구분 + 새 사용자 onboarding 시 메뉴 탐색성 개선.
  - **E2E 21단계 검증** (`scripts/test-finance-e2e.ts` · `npx tsx`) — Layer 1·5 통합 검증: AccountingConfig 생성, VAS 39 계정/14 매핑 확인, BankAccount 생성, 매출/매입/Cash/Expense 자동분개, 시산표 균형, 손익 인식, 재무상태표 A=L+E, 현금흐름표 net 검증, verify ok, close + AMB 동결, 마감 후 분개 차단(PERIOD_CLOSED throw), reopen 후 재개, K_IFRS preset 적용, AccountMapping 트리거 변경. 21/21 PASS.
- **v2.5.0 · 2026-05-03**: 재경 **Layer 4 — 재무제표 + 기간 마감** 신설.
  - 재무제표 4종 신규 화면 + API: `/finance/trial-balance` (시산표 — leaf 계정 차/대 합계 + 균형 검증), `/finance/income-statement` (손익계산서 — 수익/비용/당기순이익, 인쇄·Excel), `/finance/balance-sheet` (재무상태표 — 자산/부채/자본, 당기누적이익잉여금 자동 합산, A=L+E 균형 검증 배지), `/finance/cash-flow` (현금흐름표 — 직접법, 111/112 cash 계정의 source 별 분류).
  - 기간 마감 워크플로(verify → close → reopen): `PeriodClose` 모델 + `AccountMonthlyBalance`(월별 계정 잔액 스냅샷) + `assertPeriodOpen()` 가드 — JournalEntry 생성 시 entryDate 가 CLOSED 기간이면 즉시 throw `PERIOD_CLOSED:YYYY-MM`.
  - verify: 기간 내 모든 POSTED 분개의 차대 균형 + DRAFT 잔여 검사. close: VERIFIED 에서만 허용, AMB upsert + isFrozen=true. reopen: ADMIN 만 가능, reopen reason 필수.
  - `/admin/closings` 통합 — 신규 `Period Close (Layer 4)` 카드 + 최근 12개월 마감 이력 + 기존 record-lock(legacy) 분리 표시. 행 클릭으로 ym 자동 채움.
  - 사이드바 재경 그룹 +4 메뉴(⚖️ 시산표 / 📊 손익계산서 / 📑 재무상태표 / 💵 현금흐름표). 모든 보고서 화면에 [Excel] + [🖨 인쇄] 버튼. i18n vi/en/ko 50+ 키 추가.
