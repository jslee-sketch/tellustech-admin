...
# 3-A부. 회계 설정 마스터 (Layer 5 — `/finance/accounting-config`)

> ADMIN/MANAGER 전용. 회사별 1행. preset 변경은 즉시 모든 자동분개·재무제표 산출에 영향.

## 3-A.1 표준 프리셋 3종

| Preset | 표준 | 회계연도 | 보고통화 | VAT | 보고서 언어 |
|---|---|---|---|---|---|
| **VAS** | 베트남 회계기준 (Circular 200/2014/TT-BTC) | 1월 | VND | 10% | Tiếng Việt |
| **K_IFRS** | 한국채택국제회계기준 | 1월 | KRW | 10% | 한국어 |
| **IFRS** | 국제회계기준 (IASB) | 1월 | USD | 0% | English |

preset 카드 클릭 → confirm → 단일 POST 로 회사 설정 일괄 적용. 기존 ChartOfAccounts 시드는 VAS 기준 39 계정 — 다른 표준 적용 시 별도 시드 필요(향후 확장).

## 3-A.2 세부 설정 필드

- **enableAccrual** (발생주의 회계 사용) — false 면 현금주의(자금 이동 시점에만 회계 인식).
- **enableAutoJournal** — false 면 매출/매입/Cash/Expense/Payroll 의 자동분개 hook 모두 silent. 수동 분개만 가능.
- **enforcePeriodClose** — false 면 `assertPeriodOpen` 가드 비활성. CLOSED 기간에도 분개 가능 (감사 위험).

운영 권장: `enableAccrual=true`, `enableAutoJournal=true`, `enforcePeriodClose=true`. 수동 비활성화는 디버깅/마이그레이션 시점만.

---

# 3-B부. 자동분개 매핑 관리 (`/finance/account-mappings`)

14 트리거(SALES_REVENUE/RECEIVABLE/VAT_OUT, PURCHASE_INVENTORY/PAYABLE/VAT_IN, CASH_IN/OUT/TRANSFER, EXPENSE_OPEX/CASH, PAYROLL_SALARY/PAYABLE, RENTAL_REVENUE) → VAS 코드 매핑.

| 작업 | 결과 |
|---|---|
| 트리거 행 **[수정]** → 계정 드롭다운 → **[저장]** | `AccountMapping` upsert. `isLeaf=true` 인 leaf 계정만 선택 가능 |
| 무효 매핑 (isActive=false) | 자동분개에서 lookup null 반환 → 분개 silent skip + console.error |

**주의**: 트리거 변경은 다음 신규 트랜잭션부터 즉시 적용. 기존 분개는 재계산되지 않음. 회계연도 중간 변경은 일반적으로 회피하고, 마감 후 신규 회계연도 첫날 적용 권장.
...
| 화면 | 산출 기준 |
|---|---|
| **시산표** (`/finance/trial-balance`) | 기간 내 POSTED JournalLine 의 leaf 계정별 차/대 합계 + 차대균형 검증 |
| **손익계산서** (`/finance/income-statement`) | 기간 내 REVENUE - EXPENSE = 순이익. Excel + 인쇄 |
| **재무상태표** (`/finance/balance-sheet`) | asOf 까지 누적 ASSET = LIABILITY + EQUITY + 당기누적이익잉여금. A=L+E 균형 배지 |
| **현금흐름표** (`/finance/cash-flow`) | 직접법. 111/112 cash 계정의 source 별 분류 (현재는 모두 영업활동) |

모든 화면 [Excel] + [🖨 인쇄] (window.print + dark-mode 색상 무력화).

---

# 3-D부. 기간 마감 통합 (Layer 4 — `/admin/closings`)

기존 record-lock(legacy) 카드 + 신규 PeriodClose 카드 + 최근 12개월 이력. **워크플로**:

1. **검증 (verify)** — 기간 내 모든 POSTED 분개의 차대 균형 + DRAFT 잔여 검사. ok=true 면 status=VERIFIED.
2. **마감 (close)** — VERIFIED 에서만 가능. AccountMonthlyBalance 39행 upsert + isFrozen=true. PeriodClose.status=CLOSED. 이후 entryDate 가 해당 기간인 신규 분개는 `PERIOD_CLOSED:YYYY-MM` throw.
3. **재개 (reopen)** — ADMIN 만. reopen reason 필수 (감사 흔적). AMB unfreeze + status=REOPENED.

행 클릭으로 ym 자동 채움. 운영 권장: 매월 5일 verify → 7일 close. 재개는 회계감사 또는 신뢰 가능한 사유에 한정.

---

# 3-E부. 비용 센터 / 예산 / 거래처별 수익성 (Layer 2)

## 3-E.1 비용 센터 (`/finance/cost-centers`)

CostCenter (DEPARTMENT/BRANCH/PROJECT) + 월별 Budget. 각 비용에 cost center 할당 → 예산 대비 실적 자동 집계.

매월 1일 03:00 KST `/api/jobs/finance-monthly-snapshot` cron 이 ExpenseAllocation 합계 → `Budget.actualAmount` 채우고, 초과 시 `BUDGET_OVERRUN` 알림 발송.

## 3-E.2 거래처별 수익성 (`/finance/profitability`)

매출 - (운송비 + 부품비 + 기타직접비 + 간접비안분) = 거래처별 순이익. Excel 다운로드 지원.

`Expense.targetClientId` 가 채워져 있어야 정확한 집계 가능 — 비용 등록 시 `targetClient` 입력 필수 권장.
...
- **v2.9.1 · 2026-05-04**: 100 시나리오 E2E 시드 + 재무제표 4종 보강 + 클라이언트 5건 버그 수정. 시산표 활성 계정 9개 → 30개로 풍부화 (5111/5113/5117 매출 분리, VAT 3331/133, 6423~6429 비용 세분, 211 유형자산, 411/421/338 자본·차입). CF 분류 로직 (211 → INVESTING, 411/421/338 → FINANCING) 추가. 클라이언트 j.data 이중래퍼/URL 쿼리/cache:no-store/CF 투자·재무 섹션/프린트 색상 5건 수정. 자세한 내용은 사용자 매뉴얼 부록 K 참조.
- **v2.9.0 · 2026-05-03**: PWA 오프라인 + 포인트 만료 정책 신설. 자세한 내용은 사용자 매뉴얼 부록 K 참조.
