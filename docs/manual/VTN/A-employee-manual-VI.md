...
- **v2.9.1 · 2026-05-04**: Seed E2E 100 kịch bản + làm giàu 4 báo cáo tài chính + sửa 5 lỗi client.
  ...
  - **Phân loại lưu chuyển tiền**: `src/lib/financial-statements.ts` `computeCashFlow` — dựa vào tài khoản đối ứng của bút toán để phân INVESTING (211/212/213/214/228) vs FINANCING (311/338/411/412/421/414) vs OPERATING. Client (`cash-flow-client.tsx`) hiển thị cả 3 mục với tổng phụ.
  - **Sửa 5 lỗi client**: ① giả định `j.data?.rows` hai lớp → fallback `j.rows ?? j.data?.rows` (TB/IS/BS/CF/Profitability/Closings, 7 màn hình). ② Bỏ qua URL `?period=`/`?asOf=` → `useEffect` mount khởi tạo từ URL. ③ `cache: "no-store"` rõ ràng. ④ `useEffect` deps `[period]`/`[asOf]` (tự reload khi đổi input). ⑤ In ra số 0 → `@media print` `* { color: #000 !important; -webkit-print-color-adjust: exact; }` + ép màu đen cho class Tailwind + font monospace.
  - **Tính idempotent của seed**: chuẩn hoá `entryDate` về 2026-04-15 qua updateMany; số dư NH tính lại từ cash txn đã xác nhận; guard tái nhập hoàn ứng EXP-068; xử lý dư thừa SLS-006.
  - **Tự kiểm tra**: `scripts/dev-only/verify-100.ts` (11/11 PASS) + `http-check.sh` (65/65) + `api-data-check.sh` (18/18). Chrome live 7 màn hình PASS.
  - i18n vi/en/ko +2 khóa (fs.subtotal, fs.amount).
