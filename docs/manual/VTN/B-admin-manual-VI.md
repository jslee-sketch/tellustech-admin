...
# Phần 3-A. Cấu hình kế toán (Layer 5 — `/finance/accounting-config`)

> Chỉ ADMIN/MANAGER. 1 dòng/công ty. Đổi preset áp dụng tức thời cho mọi bút toán tự động và báo cáo tài chính.

## 3-A.1 Ba preset chuẩn

| Preset | Chuẩn | Năm tài chính | Tiền tệ | VAT | Ngôn ngữ báo cáo |
|---|---|---|---|---|---|
| **VAS** | Chuẩn KT Việt Nam (TT 200) | Tháng 1 | VND | 10% | Tiếng Việt |
| **K_IFRS** | IFRS Hàn Quốc | Tháng 1 | KRW | 10% | 한국어 |
| **IFRS** | IFRS quốc tế | Tháng 1 | USD | 0% | English |

Click thẻ preset → confirm → áp dụng đồng bộ. Hệ thống TK 39 dòng hiện tại theo VAS — chuẩn khác cần seed riêng (mở rộng tương lai).

## 3-A.2 Cài đặt chi tiết

- **enableAccrual** (kế toán dồn tích) — false → cơ sở tiền mặt.
- **enableAutoJournal** — false → tắt mọi hook bút toán tự động.
- **enforcePeriodClose** — false → vô hiệu `assertPeriodOpen` (rủi ro audit).

Khuyến nghị: tất cả = true. Tắt thủ công chỉ khi debug/migrate.

---

# Phần 3-B. Quản lý ánh xạ bút toán (`/finance/account-mappings`)

14 trigger → mã VAS. Sửa hàng → chọn TK → Lưu. Chỉ TK `isLeaf=true` mới chọn được. Đổi mapping áp dụng cho giao dịch mới — bút toán cũ không tự tính lại. Khuyến nghị: thay đổi vào đầu năm tài chính, sau khi đóng kỳ.

---

# Phần 3-C. Báo cáo tài chính (Layer 4)

| Màn hình | Cơ sở tính |
|---|---|
| **Bảng cân đối thử** | Tổng Nợ/Có theo TK leaf trong kỳ + check cân bằng |
| **Báo cáo kết quả** | DOANH THU - CHI PHÍ = LN ròng. Excel + In |
| **Bảng cân đối kế toán** | TÀI SẢN = NỢ + VỐN + LN giữ lại tích lũy. Badge A=L+E |
| **Lưu chuyển tiền tệ** | Trực tiếp. TK 111/112 phân loại theo source |

Mọi màn hình có [Excel] + [🖨 In].

---

# Phần 3-D. Đóng kỳ kế toán hợp nhất (Layer 4 — `/admin/closings`)

Card record-lock cũ + card PeriodClose mới + 12 tháng gần. **Quy trình**: verify → close → reopen.

1. **Verify**: kiểm cân bằng + draft sót.
2. **Close**: chỉ từ VERIFIED. AMB upsert + isFrozen=true. Bút toán mới có entryDate trong kỳ → throw `PERIOD_CLOSED`.
3. **Reopen**: chỉ ADMIN, bắt buộc lý do.

Khuyến nghị: ngày 5 verify, ngày 7 close.

---

# Phần 3-E. Trung tâm chi phí / Ngân sách / Lợi nhuận theo KH (Layer 2)

## 3-E.1 Trung tâm chi phí (`/finance/cost-centers`)

CostCenter (DEPARTMENT/BRANCH/PROJECT) + Budget tháng. Cron `/api/jobs/finance-monthly-snapshot` (ngày 1, 03:00 KST) tổng hợp ExpenseAllocation → `Budget.actualAmount`, vượt ngân sách → `BUDGET_OVERRUN`.

## 3-E.2 Lợi nhuận theo KH (`/finance/profitability`)

Doanh thu - (vận chuyển + linh kiện + chi phí trực tiếp khác + gián tiếp phân bổ) = LN ròng theo KH. Excel. `Expense.targetClientId` cần được điền để tính chính xác.
...
- **v2.9.5 · 2026-05-04**: Làm giàu 3 màn hình tồn kho — options/đơn giá mua TB/SL khả dụng/tình trạng. Migration DB. Thẻ chi tiết 1 S/N. Chi tiết tại Phụ lục K.
- **v2.9.4 · 2026-05-04**: Tổ chức lại logic đăng ký xuất nhập kho — S/N ưu tiên + xác minh master + 7 chính sách (A/B/D/E/G/H + match S/N-mặt hàng phía server). Chi tiết tại Phụ lục K.
- **v2.9.3 · 2026-05-04**: Trường kho đến (OUT) + xác minh master nhãn QR + backfill S/N mồ côi (123 tự đăng ký). Seed+xác minh production (created=308, toàn vẹn S/N 19/19 PASS, Chrome 7 màn hình PASS). Chi tiết tại Phụ lục K.
- **v2.9.2 · 2026-05-04**: BCTC chế độ lũy kế (YTD) + tổng nâng cấp UX quản lý tiền (bán hàng/tài khoản/cash-tx/dashboard/chi phí/lợi nhuận). Mẫu chuẩn sort/search/date/paging. Modal xác nhận thu/chi + sửa lỗi xu hướng tháng = 0 + 3 thẻ KPI dashboard từ seed 100. layout.tsx `<script>` → next/script. Chi tiết tại Phụ lục K của Sách hướng dẫn nhân viên.
- **v2.9.1 · 2026-05-04**: Seed E2E 100 kịch bản + làm giàu 4 báo cáo tài chính + sửa 5 lỗi client. TB tài khoản hoạt động 9 → 30 (chia doanh thu 5111/5113/5117, VAT 3331/133, chi phí 6423~6429, 211 TSCĐ, 411/421/338 vốn·vay). Bổ sung phân loại CF (211→INVESTING, 411/421/338→FINANCING). Sửa 5 lỗi client: j.data hai lớp / URL query / cache:no-store / CF mục đầu tư·tài chính / màu in. Chi tiết tại Phụ lục K của Sách hướng dẫn nhân viên.
- **v2.9.0 · 2026-05-03**: PWA offline + chính sách hết hạn điểm. Chi tiết tại Phụ lục K của Sách hướng dẫn nhân viên.
- **v2.8.0 · 2026-05-03**: Hệ thống thông báo tích hợp (Email + Zalo + ERP Chat) — 30 sự kiện, 27 quy tắc mặc định, dispatcher + 2 cron + 4 màn hình + badge 🔔 sidebar. Chi tiết tại Phụ lục K của Sách hướng dẫn nhân viên.
- **v2.7.1 · 2026-05-03**: Vá UX nhỏ — header cash-tx + modal duyệt hoàn ứng.
- **v2.7.0 · 2026-05-03**: 8 hạng mục tiếp theo — RBAC + thân Sách hướng dẫn hợp nhất (3-A~3-E) + thống kê kỹ sư chính xác + backfill Sales FK + đồng bộ favorite trên server. Chi tiết tại Phụ lục K của Sách hướng dẫn nhân viên.
