        ...
        <Card>
          <div className="mb-3 flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[260px]">
              <div className="mb-1 text-[11px] font-bold text-[color:var(--tts-muted)]">{t("compat.searchLabel", lang)}</div>
              <TextInput
                value={q}
                onChange={(e) => setQ(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") search(); }}
                placeholder={t("compat.searchPh", lang)}
              />
            </div>
          ...
          <div className="mt-4 space-y-4">
            {results.length === 0 && q && !busy && (
              <div className="text-center text-[12px] text-[color:var(--tts-muted)]">{t("portal.cert.empty", lang)}</div>
            )}
            {results.map((r) => (
              <div key={r.searched.id} className="rounded-md border border-[color:var(--tts-border)] bg-[color:var(--tts-card)] p-3">
                ...
                <div className="ml-3">
                  <div className="text-[11px] font-bold text-[color:var(--tts-sub)]">
                    {direction === "product_to_parts" ? t("compat.compatPartsResult", lang) : t("compat.compatProducts", lang)} ({r.compatible.length}{t("stats.casesShort", lang)})
                  </div>
                  {r.compatible.length === 0 ? (
                    <div className="mt-1 text-[11px] text-[color:var(--tts-muted)]">{t("compat.noMappingsHint", lang)}</div>
                  ) : (
                    <table className="mt-1 w-full text-[12px]">
                      <thead className="border-b border-[color:var(--tts-border)] text-[10px] text-[color:var(--tts-muted)]">
                        <tr>
                          <th className="px-2 py-1 text-left">{t("col.itemCode", lang)}</th>
                          <th className="px-2 py-1 text-left">{t("col.itemNameCol", lang)}</th>
                          <th className="px-2 py-1 text-left">{t("col.itemType", lang)}</th>
                          <th className="px-2 py-1 text-left">{t("col.description", lang)}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {r.compatible.map((c) => (
