...
import { recalcPurchaseTotalsInTx } from "@/lib/sales-recalc";
...
export async function PATCH(request: Request, context: RouteContext) {
  return withSessionContext(async () => {
    const { id: purchaseId, itemId } = await context.params;
    const existing = await prisma.purchaseItem.findUnique({ where: { id: itemId } });
    if (!existing || existing.purchaseId !== purchaseId) return notFound();
    ...
    try {
      ...
      const updated = await prisma.$transaction(async (tx) => {
        const upd = await tx.purchaseItem.update({ where: { id: itemId }, data });
        await recalcPurchaseTotalsInTx(tx, purchaseId);
        return upd;
      });
