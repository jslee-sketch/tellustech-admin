...
// 매입 사후조정 탭 — 매출과 대칭(EXCHANGE_IN 사용).
...
type Adjustment = {
  ...
  performedAt: string;
  items: { id: string; action: string; serialNumber: string; itemId: string; unitPrice: string }[];
};
...
type ExchangeLine = { itemId: string; serialNumber: string; unitPrice: string };
...
export function PurchaseAdjustmentsTab({ purchaseId, originalItems, warehouses, lang }: Props) {
  ...
  async function reload() {
    setLoading(true);
    try {
      const r = await fetch(`/api/purchases/${purchaseId}/adjustments`).then((r) => r.json());
      setHistory(
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isReturn || isExchange) {
        for (const id of selectedReturns) {
          ...
          items.push({
            ...
            itemId: orig.itemId,
            originalPurchaseItemId: orig.id,
      ...
      const res = await fetch(`/api/purchases/${purchaseId}/adjust`, {
        method: "POST",
  ...
  const histColumns: DataTableColumn<Adjustment>[] = [
    ...
    { key: "performedAt", label: t("col.effectiveDate", lang), width: "110px" },
    {
      key: "items",
    ...
    { key: "refundAmount", label: t("col.refund", lang), align: "right", width: "120px", render: (v) => formatVnd(v as string) },
    { key: "newAmount", label: t("col.newAmount", lang), align: "right", width: "120px", render: (v) => formatVnd(v as string) },
    {
      key: "netAmount",
      ...
      width: "130px",
      render: (v) => (
        <span className="font-mono font-bold text-[color:var(--tts-primary)]">{formatVnd(v as string)}</span>
      ),
    },
    {
      key: "reason",
                    ...
                    {originalItems.map((it) => (
                      <tr key={it.id} className="border-b border-[color:var(--tts-border)] last:border-0">
                        <td className="w-[40px] py-1">
                          <Checkbox checked={selectedReturns.has(it.id)} onChange={() => toggleReturn(it.id)} />
                        </td>
