...
type Amendment = {
  ...
  items: { id: string; action: string; serialNumber: string; itemId: string; salesPrice: string | null }[];
};
...
export function TmAmendmentsTab({ rentalId, originalItems, warehouses, lang }: Props) {
  ...
  const needsItems = !isTerminate;

  async function reload() {
    setLoading(true);
    try {
      const r = await fetch(`/api/rental/tm-rentals/${rentalId}/amendments`).then((r) => r.json());
      setHistory(
  ...
  function addLine() {
    setAddLines((p) => [
      ...p,
      { itemId: "", serialNumber: "", salesPrice: "", startDate: "", endDate: "" },
    ]);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/rental/tm-rentals/${rentalId}/amend`, {
        method: "POST",
          ...
          {showRemoveSelector && (
            <div className="mt-2 rounded border border-[color:var(--tts-border)] bg-[color:var(--tts-input)] p-2">
              ...
              {originalItems.length === 0 ? (
                <div className="text-[12px] text-[color:var(--tts-muted)]">No active items.</div>
              ) : (
                    ...
                    {originalItems.map((it) => (
                      <tr key={it.id} className="border-b border-[color:var(--tts-border)] last:border-0">
                        <td className="w-[40px] py-1">
                          <Checkbox checked={selectedRemove.has(it.id)} onChange={() => toggleRemove(it.id)} />
                        </td>
                        <td className="font-mono text-[11px]">{it.serialNumber}</td>
                        <td>{it.itemName}</td>
                        <td className="text-right font-mono">{formatVnd(it.salesPrice)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="mt-3 rounded border border-[color:var(--tts-border)] bg-[color:var(--tts-input)] p-2">
              ...
              {addLines.map((line, i) => (
                <Row key={i}>
                  ...
                  <Field label="Sales price" required width="160px">
                    <TextInput
                      type="number"
