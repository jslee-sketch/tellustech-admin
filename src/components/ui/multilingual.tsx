...
export function Multilingual({
  ...
  if (!primaryText) {
    return <div className="text-[12px] text-[color:var(--tts-muted)]">{t("common.noContent", currentLang)}</div>;
  }
  ...
  return (
    <div className="text-[13px]">
      <div className="mb-1 flex items-center gap-2 text-[10px] font-bold tracking-widest text-[color:var(--tts-sub)]">
        ...
        {primaryLang === originalLang && <span className="rounded bg-[color:var(--tts-accent-dim)] px-1.5 py-0.5 text-[9px] text-[color:var(--tts-accent)]">{t("common.original", currentLang)}</span>}
      </div>
      ...
      {otherLangs.length > 0 && (
        <details className="mt-3 cursor-pointer">
          <summary className="text-[11px] text-[color:var(--tts-sub)] hover:text-[color:var(--tts-text)]">
            {t("common.viewOtherLangs", currentLang).replace("{langs}", otherLangs.map((l) => NAMES[l]).join(", "))}
          </summary>
          <div className="mt-2 space-y-3 border-l-2 border-[color:var(--tts-border)] pl-3">
            {otherLangs.map((l) => (
              <div key={l}>
                <div className="mb-1 flex items-center gap-2 text-[10px] font-bold tracking-widest text-[color:var(--tts-sub)]">
                  <span>{NAMES[l]}</span>
                  {l === originalLang && <span className="rounded bg-[color:var(--tts-accent-dim)] px-1.5 py-0.5 text-[9px] text-[color:var(--tts-accent)]">{t("common.original", currentLang)}</span>}
                </div>
